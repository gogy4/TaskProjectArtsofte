...
public class JobService(IJobRepository repository, HttpClient httpClient, IJobHistoryService service, IJobHelper jobHelper) : IJobService
{
    ...
    {
        var job = await jobHelper.GetCreatedOrAssignedByUserAsync(JobMapper.ToDto(await repository.GetByIdAsync(id)), httpClient);
        return job;
    }
    ...
    public async Task<int> AssignUserAsync(int jobId, int userId)
    {
        ...
        var jobHistoryDto = new JobHistoryDto(resultId, $"Назначили исполнителя с id : {userId}", DateTime.UtcNow, jobDto.CreatedUserId);
        await service.CreateAsync(jobHistoryDto);
        return resultId;
    }
    
    public async Task<IEnumerable<JobHistoryDto>> GetHistoryByJobIdAsync(int jobId, int pageNumber = 1, int pageSize = 10)
    {
